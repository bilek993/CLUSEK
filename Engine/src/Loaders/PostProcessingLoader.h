...
#define MAP_POST_PROCESSING_EFFECT(INPUT_NAME, MAPPED_NAME, VERTEX_FILENAME, PIXEL_FILENAME) MapPostProcessingEffects(INPUT_NAME, MAPPED_NAME, VERTEX_FILENAME, PIXEL_FILENAME, format, postProcessingSettings, deviceContext, device, windowWidth, windowHeight);

class PostProcessingLoader final
{
public:
	static void Load(const std::string& path, DXGI_FORMAT format, PostProcessingSettings* postProcessingSettings, 
