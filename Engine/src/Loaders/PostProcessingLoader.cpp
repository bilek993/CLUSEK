	...
	ID3D11DeviceContext* deviceContext, ID3D11Device* device, const int windowWidth, const int windowHeight)
{
	...
	for (auto it = jsonObject.begin(); it != jsonObject.end(); ++it)
	{
		...
		MAP_POST_PROCESSING_EFFECT(postProcessingEffectName, "Chromatic Aberration", "chromatic_aberration_pixel_shader.cso");
		MAP_POST_PROCESSING_EFFECT(postProcessingEffectName, "Thirds Split", "thirds_split_pixel_shader.cso");
	}
}
